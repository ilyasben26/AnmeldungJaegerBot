...
#async def on_message(message):
  ...
  #if message.content.startswith('$bremen_test'):
    # Bremen Mitte
    #await message.channel.send('Checking Bremen ...')
    #response = check("Bremen", "Wohnsitz ummelden")
...
async def send_message():
  ...
  for entry in city_availability:
    if entry['send_msg_flag'] == True:
      ...
      channel_city = discord.utils.get(client.get_all_channels(), name=entry['city'].lower()) 
      filtered_response = [element for element in response if element['city'].lower() == entry['city'].lower()]
